    ...
    <div class="col-lg-12">
      <div class="panel panel-default">
        <div class="panel-heading">
          <input type="button" class="btn btn-default" value="新增" onclick="toAddCategory()"/>
          <input type="button" class="btn btn-default" value="删除" onclick="delCategory()"/>
        ...
        <div class="panel-body">
          <table id="priginTable" data-toggle="table" data-pagination="true">
            ...
            <c:forEach items="${applicationScope.categoryList}" var="cate">
              <tr>
                <td><input type="radio" name="originId" value="${cate.id}" onclick="document.getElementById('cateId').value=this.value;"></td>
                <td>${cate.cateNo}</td>
                <td>${cate.cateName}</td>
                <td>${cate.createEmpName}</td>
                <td>${cate.modifyEmpName}</td>
                <td><fmt:formatDate value="${cate.createTime}" pattern="yyyy-MM-dd HH:mm"/></td>
                <td><fmt:formatDate value="${cate.modifyTime}" pattern="yyyy-MM-dd HH:mm"/></td>
              </tr>
            </c:forEach>
          </table>
        </div>
...
<script>
  function toAddCategory(){
    ...
    window.open(bathPath+"common/toAddSystemParam.do?toUrl=system/addCategory.do&systemParamName=类别",
  ...
  function delCategory(){
    ...
    $.ajax({
      type:"POST",
      url:basePath+"system/delCategory.do",
      data:{cateId:cateId},
      async:false,
      error:function(){
        alert("删除失败，服务器内部错误");
